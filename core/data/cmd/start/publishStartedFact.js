...
export async function publishStartedFact({
  ...
  try {
    const current = await dataMapper.query.readStateEdgeStatus({ edgeId: stateId })
    if (statusFrom(current) === domain.edge.has_data_state.stateMachine_data.constants.Status.PROVIDED) {
  ...
  const status = domain.edge.has_data_state.stateMachine_data.constants.Status.RUNNING
  const payload = {
    instanceId,
    instanceVertexId: componentInstanceVertexId,
    stateMachineId: stateMachineVertexId,
    stateEdgeId: stateId,
    stateId,
    nodeId: dataVertexId,
  ...
  for (const field of [
    ...
    handlerDiagnostics.require(
      ...
      `${field} required before publishing data started`,
      { field },
    )
  }
  handlerDiagnostics.require(
    ...
    'deps must be an object before publishing data started',
    { field: 'deps' },
  )

  await natsContext.publish(
    createSubject(emits['domain.edge.has_data_state.started.v1']).forPublish().env('prod').build(),
