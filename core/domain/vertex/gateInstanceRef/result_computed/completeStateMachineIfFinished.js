...
async function isGateFinished({ g, dataMapper, gateInstanceRefId, cache, stateStatusOverrides, gateResultOverrides }) {
  const overrideResult = gateResultOverrides?.get(gateInstanceRefId)
  let rawResult = overrideResult
...
async function areAllGatesFinished({ g, dataMapper, instanceVertexId, cache, stateStatusOverrides, gateResultOverrides }) {
  ...
  for (const gateInstanceRefId of gateInstanceRefIds ?? []) {
    ...
    const finishedGate = await isGateFinished({ g, dataMapper, gateInstanceRefId, cache, stateStatusOverrides, gateResultOverrides })
...
async function isInstanceFinished({ g, dataMapper, instanceVertexId, stateMachineId, cache, stateStatusOverrides, gateResultOverrides }) {
  ...
  const gatesFinished = await areAllGatesFinished({ g, dataMapper, instanceVertexId, cache, stateStatusOverrides, gateResultOverrides })
  if (!gatesFinished) {
    cache.set(cacheKey, false)
    return false
  }
...
async function completeInstanceChain({ g, dataMapper, emits, natsContext, instanceVertexId, stateMachineId, instanceId, visited, finishedCache, stateStatusOverrides, gateResultOverrides }) {
  ...
  const finished = await isInstanceFinished({ g, dataMapper, instanceVertexId, stateMachineId, cache: finishedCache, stateStatusOverrides, gateResultOverrides })
...
export async function completeStateMachineIfFinished({
  ...
  if (gateInstanceRefId) {
    const serializedGateResult = typeof resultValue === 'string'
      ? resultValue
      : (result != null ? JSON.stringify(result) : '')
    gateResultOverrides.set(gateInstanceRefId, serializedGateResult)
  }
